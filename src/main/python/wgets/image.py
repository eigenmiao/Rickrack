            ...
            if isinstance(self._locating_img, tuple):
                ...
                if revised_locating_value and value[4]:
                    ...
                    if value[0] == 1:
                        for i in range(3):
                            if self._args.sys_color_set[self._args.sys_activated_idx].rgb[i] > rgb[i]:
                                ...
                                fact.append((self._args.sys_color_set[self._args.sys_activated_idx].rgb[i] - rgb[i]) / (255 - (rgb[i] - 0.00001)))
                            else:
                                separ.append(rgb[i] + 0.00001 + (255.0 - rgb[i]) * value[3])
                                fact.append((rgb[i] - self._args.sys_color_set[self._args.sys_activated_idx].rgb[i]) / (rgb[i] + 0.00001))
                    ...
                    if value[0] == 1:
                        for i in range(3):
                            if self._args.sys_color_set[self._args.sys_activated_idx].rgb[i] > rgb[i]:
                                fori = (self._args.sys_color_set[self._args.sys_activated_idx].rgb[i] - rgb[i]) / (255.0 - (rgb[i] - 0.00001))
                                fact.append(fori * value[3])
                            else:
                                fori = (rgb[i] - self._args.sys_color_set[self._args.sys_activated_idx].rgb[i]) / (rgb[i] + 0.00001)
                                fact.append(fori * value[3])
